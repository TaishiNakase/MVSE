...
#'
#' An object of class \code{mvsefit} contains the output derived from sampling of a
...
#' @slot mvsemodel The instance of S4 class \code{\linkS4class{mvsemodel}}.
...
#' \describe{\item{\code{extract}}{Extact the sampled posterior distribution of the Index P as well as the ecological scaling parameters.}}}
...
#' @param model_category A character string naming the model category. Either \code{"user_defined} or \code{"denv_aegypti}. 
#' @param mvsemodel An object of class \code{\linkS4class{mvsemodel}} used for the sampling procedure.
...
setMethod("show", "mvsefit",
          function(object) {
            ...
            if (object@model_category=="denv_aegypti") vars <- c("rho", "eta", "indexP")
            ...
            compute_summary_stats <- function(x) {
              ...
              if (x %in% all_gen_pars) {
                n_eff <- NA; se_mean <- NA; n_eff <- NA
              } else {
                n_eff <- coda::effectiveSize(draw); names(n_eff) <- NULL
...
setMethod("print", "mvsefit",
          function(x, pars=NULL, probs=c(0.25, 0.5, 0.75), digits_summary=3, ...) {
            ...
            if (is.null(pars)) {
              if (x@model_category=="denv_aegypti") pars <- c("rho", "eta", "indexP")
...
##########################################################################################################
#' Extract samples from a \code{\linkS4class{mvsefit}} object.
#'
#' Extract samples from a sampled mvse model represented by an instance of class \code{\linkS4class{mvsefit}}.
#'
#' @param object An object of class \code{\linkS4class{mvsefit}}.
#' @param pars A character vector of parameter names. The default is all parameters
#' (i.e. \code{"rho"}, \code{"eta"}, \code{"alpha"}, \code{"indexP"}).
#'
#' @return A named list, every element of which is either a numeric vector (\code{"rho"}, \code{"eta"}, \code{"alpha"}) or
#' a dataframe (\code{"indexP"}) representing samples for a parameter.
#'
#' @examples
#' # obtain climate data
#' data(climateFSA)
...
#' head(indexP_output$indexP[, 1:11])
#'
#' @usage NULL
#' @name extract
#' @export
...
setMethod("extract", "mvsefit",
          function(object, pars=NULL) {
            if (is.null(pars)) {
              if (object@model_category=="denv_aegypti") pars <- c("rho", "eta", "indexP")
              else pars <- c("rho", "eta", "alpha", "indexP")
            }
...
setMethod("mcmc_pairs", "mvsefit",
          function(object, filename=NULL, options=list(max_draws=10^4)) {
            if (object@model_category=="denv_aegypti") factor_names <- c("rho", "eta")
            ...
            for (ii in seq_along(factor_names)) {
              for (jj in seq_along(factor_names)) {
                ...
                p <- ggplot(data=p_df) +
                  theme_bw() +
                  geom_point(aes_string(x="x", y="y"), color=pal[4], fill=pal[3], shape=21, size=0.5) +
...
#'
#' # run the MCMC sampling procedure to estimate the epi-entomological parameters as well Index P.
#' aegypti_fit <- sampling(object=aegypti_model, iter=10^5, warmup=0.2, seed=123, 
#' init=c(rho=1, eta=10, alpha=3), samples=1000)
#'
...
setMethod("mcmc_traceplot", "mvsefit",
          function(object, filename=NULL, options=list(max_iter=10^4)) {
            ...
            for (ii in seq_along(factor_names)) {
              ...
              p <- ggplot(p_df) +
                theme_bw() +
                geom_line(aes_string(x="x", y="y"), color=pal[3]) +
                labs(title=factor_names[ii]) +
...
#' \code{"eta"} and \code{"alpha"}) to include in the plots. If NULL (default), all factors are included.
#' @param filename The name of the file where the plot in PNG format is saved (optional).
#'
#' @return Several ggplot objects organized into a grid via \code{\link{bayesplot_grid}} is returned.
#'
#' @examples
#' # obtain climate data
#' data(climateFSA)
...
setMethod("mcmc_factor_dist", "mvsefit",
          function(object, factors=NULL, filename=NULL) {
            if (object@model_category=="denv_aegypti") {
              factor_names <- c("rho", "eta")
            } else {
              factor_names <- c("rho", "eta", "alpha")
            }
            if (!is.null(factors)) {
              this_factors <- MVSE::extract(object)[factors]
            } else {
              this_factors <- MVSE::extract(object)[factor_names]; factors <- factor_names
            }
...
setMethod("mcmc_index_dist", "mvsefit",
          function(object, index="indexP", filename=NULL, options=list(smoothing=NULL)) {
            ...
            index_df <- gather(data=index_df, key="percentile", value="value", 2:ncol(index_df)) %>%
              mutate(color=ind[.data$percentile])
            ...
            if (!is.null(smoothing)) {
              ...
              index_smooth <- gather(index_smooth, key="variable", value="value", 2:ncol(index_smooth))
            }
            ...
            p <- ggplot(data=index_df) +
              geom_line(aes_string(x="date", y="value", group="percentile", color="color", alpha="color")) +
              theme_bw() +
              labs(y=index) +
            ...
            if (!is.null(smoothing))
              p <- p + geom_line(data=index_smooth, aes_string(x="date", y="value", group="variable"), color="black", size=1.2)
            if (!is.null(filename)) {
              png(filename, width = 1024, height = 768, res=150)
              print(p)
              dev.off()
