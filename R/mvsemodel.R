...
#' for  human/vector ecological/epidemiological parameters. The \code{fitting}
#' method defined in this class may be used to draw samples of the ecological factors
...
#' @slot priors The prior distributions of the human/vector ecological/epidemiological parameters,
...
#'
#' @details
#' The \code{priors} argument requires data in a specific format. The list must contain five lists \code{mosq_life_exp}, \code{mosq_inc_per},
...
#' human incubation period (days) respectively. The lists for each of the five distributions
...
#'
#' @return An instance of S4 class \code{\linkS4class{mvsemodel}} that can be passed to \code{\link{fitting}}.
#' 
#' @examples 
...
#' 
#' # Pre-defined model with priors for dengue virus transmission by Aedes aegypti mosquitoes in human hosts. 
...
mvse_model <- function(model_name="anon_model", model_category="user-defined", climate_data=data.frame(), priors=list(), warning=TRUE) {
  if (model_category=="aegypti") {
    if (warning) print("Model category is 'aegypti' so user-defined prior distributions will be ignored")
...
setMethod("show", "mvsemodel",
          function(object) {
            ...
            if (object@model_category=="aegypti") {
              cat("\t Mosquito incubation period (days) \t\t \n")
              for (i in seq_along(object@priors$mosq_inc_per$temps)) {
                temp <- object@priors$mosq_inc_per$temps[i]
                if (i==1) temp <- paste0("  <", temp, "\u00B0C", sep="")
                else if (i==length(object@priors$mosq_inc_per$temps)) temp <- paste("  >", object@priors$mosq_inc_per$temps[i-1], "\u00B0C", sep="")
                else temp <- paste(object@priors$mosq_inc_per$temps[i-1], "-", temp, "\u00B0C", sep="")
                description <- .getDistDesc(pars=object@priors$mosq_inc_per$dists[[i]]$pars, dist=object@priors$mosq_inc_per$dists[[i]]$dist)
...
##########################################################################################################
#' Print default summary of model
#'
...
setMethod("print", "mvsemodel",
          function(x, ...) {
            ...
            if (x@model_category=="aegypti") {
              cat("\t Mosquito incubation period (days) \t\t \n")
              for (i in seq_along(x@priors$mosq_inc_per$temps)) {
                temp <- x@priors$mosq_inc_per$temps[i]
                if (i==1) temp <- paste0("  <", temp, "\u00B0C", sep="")
                else if (i==length(x@priors$mosq_inc_per$temps)) temp <- paste("  >", x@priors$mosq_inc_per$temps[i-1], "\u00B0C", sep="")
                else temp <- paste(x@priors$mosq_inc_per$temps[i-1], "-", temp, "\u00B0C", sep="")
                description <- .getDistDesc(pars=x@priors$mosq_inc_per$dists[[i]]$pars, dist=x@priors$mosq_inc_per$dists[[i]]$dist)
                cat("\t\t\t\t\t", temp, "\t:", description, "\n")
              }
            }
            else {
...
##########################################################################################################
#' Set (or replace) the prior distribution of a human/vector ecological/epidemiological parameter.
#'
#' @param object An object of class \code{\linkS4class{mvsemodel}}.
#' @param name A character string specifying the parameter whose distribution is to be set (and replaced).
...
#' \code{"human_inf_per"}.
#' @param dist A character string specifying the distribution name. Currently only normal and lognormal distributions
#' are supported. 
...
#'
#' @return An instance of S4 class \code{\linkS4class{mvsemodel}} with the prior distribution updated for
#' the specified human/vector ecological/epidemiological parameters.
#' #' # Daily temperature, relative humidity and precipitation data from 1981 in 2019 in the 
#' # city of Feira de Santana in Bahia, Brazil.
#' 
#' @examples
...
#' # (e.g. Mosquito life expectancy to normal distribution with mean 12 days and standard deviation 2 days)
#' set_prior(object=user_model, name="mosq_life_exp", dist="normal", pars=c(mean=12, sd=2))
#' 
#' # replace previously set prior 
#' # (e.g. Mosquito life expectancy to normal distribution with mean 15 days and standard deviation 4 days)
#' set_prior(object=user_model, name="mosq_life_exp", dist="normal", pars=c(mean=15, sd=4))
#' 
#' # set 
#' @usage NULL
#' @name set_prior
...
setMethod("set_prior", "mvsemodel",
          function(object, name, dist, pars){
            if (object@model_category=="aegypti")
              stop("Model category is 'aegypti': cannot change priors")
...
#'
#' @return An object of S4 class \code{mvsefit} representing the fitted results along with additional relevant data.
#' 
#' @examples 
#' # obtain climate data
#' data(climateFSA)
...
setMethod(f="fitting",
          ...
          definition=function(object, iter=10^5, warmup=0.5, seed=sample.int(.Machine$integer.max, 1),
            ...
            mvse_data$Z<- .mvse_temp_effect_muV(mvse_data$T)
            mvse_data$Y<- .mvse_hum_effect_muV(mvse_data$H, mean(mvse_data$H))
            ...
            this_mvsefit <- mvse_fit(model_name=object@model_name, model_category=object@model_category,
                                     ...
                                     sim=c(list(accepted=mcmc_output$count_accepted, mvse_data=mvse_data),
                                           gen_quantities, posterior_list))
            return(this_mvsefit)
          }
)
...
#'
#' The default plot shows the time series for temperature and humidity. The method can also be used to
#' plot other combinations of climatic variables (i.e. temperature, humidity and rainfall). 
#'
#' @param object An object of class \code{\linkS4class{mvsemodel}}.
          ...
          definition=function(object, vars=c("temperature", "humidity"), smoothing=NULL, filename=NULL) {
            ...
            if (length(vars)==1) {
              if (vars=="temperature") {
                p <- ggplot(data) +
                  theme_bw() +
                  geom_line(aes(x=date, y=T), color='cadetblue4') +
                ...
                p <- ggplot(data) +
                  theme_bw() +
                  geom_line(aes(x=date, y=H), color='magenta') +
...
##########################################################################################################
#' Plot the prior distributions of the host/vector ecological/epidemiological parameters.
#'
...
setMethod(f="plot_priors",
          ...
          definition=function(object, pars=NULL, filename=NULL) {
            ...
            for (par in pars) {
              if (!(par %in% pars)) next
              if (par=="mosq_inc_per" && object@model_category=="aegypti") {
                values_list <- list()
                densities_list <- list()
                temp_list <- list()
...
.sample_generated_quantities <- function(posterior_list, mvse_data, mvsemodel, n, verbose=FALSE){
  ...
  if (mvsemodel@model_category!="aegypti") sMCMC_alpha <- posterior_list$alpha[ss]
  ...
  if (mvsemodel@model_category=="aegypti") {
  ...
  for (ii in 1:n){
    ...
    gammaH <- gammaHs[ii]
    deltaH <- deltaHs[ii]
    muV_t <- eta*mvse_data$Z*(1+mvse_data$Y)^rho

    # calculate ento-epi parameters
    muV_t <- eta*mvse_data$Z*(1+mvse_data$Y)^rho
    if (mvsemodel@model_category!="aegypti") {
      alpha <- sMCMC_alpha[ii]
      gammaV_t <- alpha*mvse_data$G
    }
