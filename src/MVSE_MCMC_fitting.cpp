...
// [[Rcpp::export(.MVSErunMCMC_RhoEtaAlpha)]]
NumericMatrix MVSErunMCMC_RhoEtaAlpha(String model_category, unsigned int iter, Rcpp::NumericVector& init, Rcpp::NumericVector& gauJump, 
  ...
  // climate effect data
  Rcpp::NumericVector temp_muV = mvse_data["temp_muV"];
  Rcpp::NumericVector hum_muV = mvse_data["hum_muV"];
  Rcpp::NumericVector hum_aV = mvse_data["hum_aV"];
  Rcpp::NumericVector temp_gammaV = mvse_data["temp_gammaV"];
  ...
  for (int ii = 0; ii < iter; ii++) {
    ...
    for (size_t jj = 0; jj < mvse_data_num_rows; jj++) {
      mu += 1/(pEta*temp_muV[jj]*pow((1+hum_muV[jj]), pRho));
      a += prior_mosq_biting_freq_mean*pow((1+hum_aV[jj]), pRho);
    }
    ...
    p3 = 0;
    if (model_category!="denv_aegypti") p3 = mosq_inc_per_pdf(gamma, mosq_inc_per_pars);
    pL = p1 + p2 + p3;
    pAccept = exp(pL-cL);
